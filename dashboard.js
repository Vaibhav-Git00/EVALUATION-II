document.addEventListener("DOMContentLoaded", function () {
  class User {
    ...
    static async loadFromLocalStorage(email) {
  ...
  async function initializeDashboard() {
    ...
    let currentUser = await User.loadFromLocalStorage(currentEmail);
    ...
    if (!currentEmail) {
      ...
      document.getElementById(
        "welcome-user"
      ).textContent = `Hi 👋, ${currentUser.email}`;
      updateDashboard();

      //  console.log(currentUser)

      function updateDashboard() {
        document.getElementById("balance").textContent = currentUser
          .getBalance()
          .toFixed(2);
        document.getElementById("income").textContent =
          currentUser.income.toFixed(2);
        document.getElementById("expense").textContent =
          currentUser.expense.toFixed(2);
        updateHistoryTable();
      }
        ...
        sortedTransactions.forEach((transaction) => {
          ...
          editButton.addEventListener("click", function () {
            showCustomPrompt(
              "Edit Transaction",
              transaction,
              (newAmount, newCategory, newReason) => {
                const editSuccess = currentUser.editTransaction(
                  transaction.id,
                  newAmount,
                  newCategory,
                  newReason
                ...
                if (editSuccess) {
                  ...
                  showCustomAlert(
                    "Transaction cannot be processed. Balance would go negative."
                  );
                }
              }
            );
      ...
      document
        ...
        .addEventListener("submit", function (event) {
          ...
          if (amount && category && reason) {
            ...
            const transactionSuccess = currentUser.addTransaction(
              amount,
              category,
              reason
            );
      ...
      document
        ...
        .addEventListener("click", function () {
          const historyContent = document
            .getElementById("history-table")
            .cloneNode(true);
          const actionsHeader = historyContent.querySelector(
            "thead th:last-child"
          );
          if (actionsHeader) actionsHeader.remove();

          const actionCells = historyContent.querySelectorAll(
            "tbody td:last-child"
          );
